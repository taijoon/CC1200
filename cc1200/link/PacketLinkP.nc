...
implementation {
  ...
  command void PacketLink.setRetries(message_t *msg, uint16_t maxRetries) {
    (call CC1200PacketBody.getMetadata(msg))->maxRetries = maxRetries;
  }
  ...
  command void PacketLink.setRetryDelay(message_t *msg, uint16_t retryDelay) {
    (call CC1200PacketBody.getMetadata(msg))->retryDelay = retryDelay;
  }
  ...
  command uint16_t PacketLink.getRetries(message_t *msg) {
    return (call CC1200PacketBody.getMetadata(msg))->maxRetries;
  }
  ...
  command uint16_t PacketLink.getRetryDelay(message_t *msg) {
    return (call CC1200PacketBody.getMetadata(msg))->retryDelay;
  }
  ...
  void signalDone(error_t error) {
    ...
    if((call CC1200PacketBody.getMetadata(currentSendMsg))->maxRetries > 0)
	    (call CC1200PacketBody.getMetadata(currentSendMsg))->maxRetries = totalRetries;
