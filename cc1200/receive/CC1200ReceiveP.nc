...
*/
  ...
  task void waitDecTask(){
    ...
    if(decLoopCount > 10){
      call CSN.clr();
      atomic call SECCTRL0.write((0 << CC1200_SECCTRL0_SEC_MODE) |
				 (0 << CC1200_SECCTRL0_SEC_M) |
				 (0 << CC1200_SECCTRL0_SEC_RXKEYSEL) |
				 (1 << CC1200_SECCTRL0_SEC_CBC_HEAD) |
				 (1 << CC1200_SECCTRL0_RXFIFO_PROTECTION)) ;
      call CSN.set();
      SECURITYLOCK = 0;
      call SpiResource.release();
      atomic flush_flag = 1;
      beginReceive();
    }else if(status & CC1200_STATUS_ENC_BUSY){
      post waitDecTask();
    }else{
      call CSN.clr();
      atomic call SECCTRL0.write((0 << CC1200_SECCTRL0_SEC_MODE) |
  ...
  void waitDec(){
    ...
    if(status & CC1200_STATUS_ENC_BUSY){
  ...
  void dec(){
    ...
    if (pos + sizeof(cc1200_header_t) > RXFIFO_SIZE){
      temp = RXFIFO_SIZE - pos;
      call CSN.clr();
      atomic call RXFIFO_RAM.read(pos,(uint8_t*)&header, temp);
      call CSN.set();
      call CSN.clr();
      atomic call RXFIFO_RAM.read(0,(uint8_t*)&header+temp, sizeof(cc1200_header_t)-temp);
      call CSN.set();
    }else{
      call CSN.clr();
      atomic call RXFIFO_RAM.read(pos,(uint8_t*)&header, sizeof(cc1200_header_t));
      call CSN.set();
    }
	...
	if(!(header.dest==call CC1200Config.getShortAddr() || header.dest==AM_BROADCAST_ADDR)){
	  packetLength = header.length + 1;
	  m_state = S_RX_LENGTH;
	  call SpiResource.release();
	  beginReceive();
	...
	if(mode < 4 && mode > 0) { // if mode is valid
	  ...
	  if(mode == CC1200_CBC_MAC || mode == CC1200_CCM){
	    authentication = 1;
	    call CSN.clr();
	    atomic call SECCTRL0.write((mode << CC1200_SECCTRL0_SEC_MODE) |
				       ((micLength-2)/2 << CC1200_SECCTRL0_SEC_M) |
				       (key << CC1200_SECCTRL0_SEC_RXKEYSEL) |
				       (1 << CC1200_SECCTRL0_SEC_CBC_HEAD) |
				       (1 << CC1200_SECCTRL0_RXFIFO_PROTECTION)) ;
	    call CSN.set();
	  }else{
  ...
  async event void RXFIFO.readDone( uint8_t* rx_buf, uint8_t rx_len,
    ...
    case S_RX_FCF:
       ...
       */
      if(call CC1200Config.isAutoAckEnabled() && !call CC1200Config.isHwAutoAckDefault()) {
   ...
   */
  task void receiveDone_task() {
    cc1200_metadata_t* metadata = call CC1200PacketBody.getMetadata( m_p_rx_buf );
    cc1200_header_t* header = call CC1200PacketBody.getHeader( m_p_rx_buf);
