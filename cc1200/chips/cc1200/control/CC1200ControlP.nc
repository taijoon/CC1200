...
//  uses interface CC1200Register as IOCFG1;
  ...
  uses interface CC1200Register as IOCFG1;
  uses interface CC1200Register as IOCFG0;
  uses interface CC1200Register as SYNC3;
  uses interface CC1200Register as SYNC2;
  uses interface CC1200Register as SYNC1;
  uses interface CC1200Register as SYNC0;
  uses interface CC1200Register as SYNC_CFG1;
  uses interface CC1200Register as SYNC_CFG0;
  ...
  uses interface CC1200Register as PREAMBLE_CFG1;
  ...
  uses interface CC1200Register as AGC_GAIN_ADJUST;
  uses interface CC1200Register as AGC_CFG3;
  uses interface CC1200Register as AGC_CFG2;
  ...
  uses interface CC1200Register as DEV_ADDR;
  uses interface CC1200Register as SETTLING_CFG;
  uses interface CC1200Register as FS_CFG;
  uses interface CC1200Register as WOR_CFG1;
  uses interface CC1200Register as WOR_CFG0;
  uses interface CC1200Register as WOR_EVENT0_MSB;
  uses interface CC1200Register as WOR_EVENT0_LSB;
  uses interface CC1200Register as RXDCM_TIME;
  uses interface CC1200Register as PKT_CFG2;
  uses interface CC1200Register as PKT_CFG1;
  uses interface CC1200Register as PKT_CFG0;
  uses interface CC1200Register as RFEND_CFG1;
  uses interface CC1200Register as RFEND_CFG0;
  uses interface CC1200Register as PA_CFG1;
  uses interface CC1200Register as PA_CFG0;
  uses interface CC1200Register as ASK_CFG;
  ...
  uses interface CC1200Register as TOC_CFG;
  uses interface CC1200Register as MARC_SPARE;
  uses interface CC1200Register as ECG_CFG;
  uses interface CC1200Register as MDMCFG2;
  uses interface CC1200Register as EXT_CTRL;
  uses interface CC1200Register as RCCAL_FINE;
  uses interface CC1200Register as RCCAL_COARSE;
  uses interface CC1200Register as RCCAL_OFFSET;
  uses interface CC1200Register as FREQOFF1;
  uses interface CC1200Register as FREQOFF0;
  uses interface CC1200Register as FREQ2;
  uses interface CC1200Register as FREQ1;
  uses interface CC1200Register as FREQ0;
  uses interface CC1200Register as IF_ADC2;
  uses interface CC1200Register as IF_ADC1;
  uses interface CC1200Register as IF_ADC0;
  uses interface CC1200Register as FS_DIG1;
  uses interface CC1200Register as FS_DIG0;
  uses interface CC1200Register as FS_CAL3;
  uses interface CC1200Register as FS_CAL2;
  ...
  uses interface CC1200Register as FS_DSM1;
  uses interface CC1200Register as FS_DSM0;
  uses interface CC1200Register as FS_DVC1;
  ...
  uses interface CC1200Register as FS_VCO4;
  uses interface CC1200Register as FS_VCO3;
  uses interface CC1200Register as FS_VCO2;
  uses interface CC1200Register as FS_VCO1;
  uses interface CC1200Register as FS_VCO0;
  uses interface CC1200Register as GBIAS6;
  uses interface CC1200Register as GBIAS5;
  uses interface CC1200Register as GBIAS4;
  uses interface CC1200Register as GBIAS3;
  uses interface CC1200Register as GBIAS2;
  uses interface CC1200Register as GBIAS1;
  uses interface CC1200Register as GBIAS0;
  uses interface CC1200Register as IFAMP;
  uses interface CC1200Register as LNA;
  uses interface CC1200Register as RXMIX;
  uses interface CC1200Register as XOSC5;
  uses interface CC1200Register as XOSC4;
  uses interface CC1200Register as XOSC3;
  uses interface CC1200Register as XOSC2;
...
implementation {
  ...
  async command error_t CC1200Power.startOscillator() {
    ...
    call SpiResource.request();
		// Register Write
    call CSN.clr();  call IOCFG3.write(0x0011);		call CSN.set();
    call CSN.clr();  call IOCFG2.write(0x0006);		call CSN.set();
    call CSN.clr();  call SYNC_CFG1.write(0x00A8);		call CSN.set();
    call CSN.clr();  call DEVIATION_M.write(0x0047);    call CSN.set();
    call CSN.clr();  call MODCFG_DEV_E.write(0x002F);    call CSN.set();
    call CSN.clr();  call DCFILT_CFG.write(0x001E);    call CSN.set();

    call CSN.clr();  call PREAMBLE_CFG1.write(0x0014);    call CSN.set();

    call CSN.clr();  call PREAMBLE_CFG0.write(0x008A);    call CSN.set();
    call CSN.clr();  call IQIC.write(0x00);    call CSN.set();
    call CSN.clr();  call CHAN_BW.write(0x01);    call CSN.set();
    call CSN.clr();  call MDMCFG1.write(0x42);    call CSN.set();
    call CSN.clr();  call MDMCFG0.write(0x05);    call CSN.set();
    call CSN.clr();  call SYMBOL_RATE2.write(0xC9);    call CSN.set();
    call CSN.clr();  call SYMBOL_RATE1.write(0x99);    call CSN.set();
    call CSN.clr();  call SYMBOL_RATE0.write(0x99);    call CSN.set();
    call CSN.clr();  call AGC_REF.write(0x2F);    call CSN.set();
    call CSN.clr();  call AGC_CS_THR.write(DDBM+10);  call CSN.set();  // -70dbm
    //call CSN.clr();  call AGC_CS_THR.write(0x01);  call CSN.set();  // -80dbm
    call CSN.clr();  call AGC_CFG2.write(0x60);    call CSN.set();
    call CSN.clr();  call AGC_CFG1.write(0x12);    call CSN.set();
    call CSN.clr();  call AGC_CFG0.write(0x84);    call CSN.set();
    ...
    call CSN.clr();  call RFEND_CFG1.write(0x3F);    call CSN.set();
    call CSN.clr();  call RFEND_CFG0.write(0x30);    call CSN.set();
    ...
    call CSN.clr();  call FREQOFF_CFG.write(0x23);    call CSN.set();
    call CSN.clr();  call MDMCFG2.write(0x00);    call CSN.set();
    call CSN.clr();  call FREQ2.write(0x5C);    call CSN.set();
    call CSN.clr();  call FREQ1.write(0x0F);    call CSN.set();
    call CSN.clr();  call FREQ0.write(0x5C);    call CSN.set();
    call CSN.clr();  call IF_ADC1.write(0xEE);    call CSN.set();
    call CSN.clr();  call IF_ADC0.write(0x10);    call CSN.set();

    call CSN.clr();  call FS_DIG1.write(0x04);    call CSN.set();
    call CSN.clr();  call FS_DIG0.write(0xA3);    call CSN.set();
    ...
    call CSN.clr();  call FS_REG_DIV_CML.write(0x1C);    call CSN.set();
    call CSN.clr();  call FS_SPARE.write(0xAC);    call CSN.set();

    call CSN.clr();  call FS_VCO0.write(0xB5);    call CSN.set();
    call CSN.clr();  call IFAMP.write(0x05);    call CSN.set();
